...
class CodePickerWidgetState extends State<CodePickerWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.builder != null) {
      internalWidget = InkWell(onTap: showCodePickerWidgetDialog, child: widget.builder!(selectedItem));
    } else {
          ...
          child: Flex(
            ...
            children: <Widget>[
              ...
              if (!widget.hideMainText)
                Flexible(
                  fit: widget.alignLeft ? FlexFit.tight : FlexFit.loose,
                  child: Text(widget.showOnlyCountryWhenClosed ? selectedItem!.toCountryStringOnly() : selectedItem.toString(), style: widget.textStyle ?? Theme.of(context).textTheme.labelLarge, overflow: widget.textOverflow),
                ),
            ],
          ),
        ),
    ...
    if (oldWidget.initialSelection != widget.initialSelection) {
      if (widget.initialSelection != null) {
        selectedItem = elements.firstWhere(
          (criteria) => (criteria.code!.toUpperCase() == widget.initialSelection!.toUpperCase()) || (criteria.dialCode == widget.initialSelection) || (criteria.name!.toUpperCase() == widget.initialSelection!.toUpperCase()),
          orElse: () => elements[0],
        );
      } else {
  ...
  void initState() {
    ...
    if (widget.initialSelection != null) {
      selectedItem = elements.firstWhere(
        (item) => (item.code!.toUpperCase() == widget.initialSelection!.toUpperCase()) || (item.dialCode == widget.initialSelection) || (item.name!.toUpperCase() == widget.initialSelection!.toUpperCase()),
        orElse: () => elements[0],
      );
    } else {
      selectedItem = elements[0];
    ...
    favoriteElements = elements.where((item) => widget.favorite.firstWhereOrNull((criteria) => item.code!.toUpperCase() == criteria.toUpperCase() || item.dialCode == criteria || item.name!.toUpperCase() == criteria.toUpperCase()) != null).toList();
  }
      ...
      builder: (context) => Center(
        child: Dialog(
          child: SelectionDialog(
            ...
            headerTextStyle: TextStyle(fontSize: 16.0, color: Theme.of(context).textTheme.bodyLarge!.color),
            topBarPadding: EdgeInsets.all(2.0),
          ),
        ),
      ),
    );
